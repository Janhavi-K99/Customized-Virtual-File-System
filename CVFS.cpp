...
void man(char *name)
{
    ...
    {
        printf("Used to display information of file\n");
    ...
    else if(strcmp(name,"cloasall") == 0)
    {
...
int CreateFile(char *name, int permission)
{
    ...
    if (temp) {
#ifndef CVFS_STANDALONE
        set_bit(g_inodeBitmap, temp->InodeNumber - 1, 0);
#endif
    }
...
int rm_File(char * name)
{
    ...
    for (int fd = 0; fd < 50; fd++) {
        if (UFDTArr[fd].ptrfiletable != NULL &&
            UFDTArr[fd].ptrfiletable->ptrinode == node) {
    ...
    SUPERBLOCKobj.FreeInode++;
#ifndef CVFS_STANDALONE
    free_inode_bit(node->InodeNumber);
#endif
...
int truncate_File(char *name)
{
    ...
    PINODE node = UFDTArr[fd].ptrfiletable->ptrinode;
